...
public class PacMan extends JPanel implements ActionListener, KeyListener {
    ...
    private void drawHeader(Graphics g) {
        int headerHeight = tileSize; // Height of the header row
        g.setColor(Color.BLACK);
        g.fillRect(0, 0, boardWidth, headerHeight);

        // Display "Lives:" text
        g.setColor(Color.WHITE);
        g.setFont(new Font("Arial", Font.BOLD, 18));
        String livesText = "Lives: " + lives;
        g.drawString(livesText, 10, headerHeight / 2 + 6); // Adjust Y to center vertically

        // Display score on the top-right corner
        String scoreText = "Score: " + score;
        int scoreWidth = g.getFontMetrics().stringWidth(scoreText);
        g.drawString(scoreText, boardWidth - scoreWidth - 10, headerHeight / 2 + 6);
    }

    public void draw(Graphics g) {
        int headerHeight = tileSize; // Adjust for the header row
        g.translate(0, tileSize); // Shift drawing below the header
